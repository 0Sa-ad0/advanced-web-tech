...
const Upload = () => {
    ...
    const handleUpload = async () => {
        if (!selectedFile) {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Please select an jpg, webp, png, jpeg, txt, pdf, xlsx, or docx file to upload and smaller than 1 GB.'
            });
            return;
        }

        const maxFileSize = 1 * 1024 * 1024 * 1024; // 1 GB in bytes
        if (selectedFile.size > maxFileSize) {
            Swal.fire({
                icon: 'error',
                title: 'File too large',
                text: 'Please select a file smaller than 1 GB.'
            });
            return;
        }
        ...
        if (!fileExtension) {
            Swal.fire({
                icon: 'error',
                title: 'Invalid file',
                text: 'Please upload a file with a valid extension.'
            });
            return;
        }
        const allowedExtensions = ['jpg', 'webp', 'png', 'jpeg', 'txt', 'pdf', 'xlsx', 'docx'];
        if (!allowedExtensions.includes(fileExtension)) {
            Swal.fire({
                icon: 'error',
                title: 'Invalid file',
                text: 'Please upload a jpg, webp, png, jpeg, txt, pdf, xlsx, or docx file and smaller than 1 GB.'
            });
            return;
        }
    ...
    return (
        <>
            ...
            <div className="flex items-center justify-center min-h-screen" style={{ backgroundImage: 'url("/img6.jpg")', backgroundSize: 'cover', backgroundPosition: 'center' }}>
